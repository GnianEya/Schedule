...
userdetails:any;
  ...
  ngOnInit() {
    ...
    this.HttpService.getMethod('http://localhost:8081/image/getImage?id='+this.currentUserID).subscribe(
      async (response) => {
  ...
  edit(){
    ...
    if(this.isEdit){
      this.HttpService.getMethod('http://localhost:8081/user/serchUserProfile?userId='+this.currentUserID)
    .subscribe(
     async (response) => {
...
onValueChange(value){
  ...
  upload(){
    ...
    this.http.put<any>('http://localhost:8081/user/updateUserProfile?password='+this.dialogData,this.editProfileSenderDto).subscribe(
      (result)=>{
